...
export class BlocklastScene extends Container {
  ...
  private updateBrickPreview(): void {
    ...
    if (this.selectedBrickIndex >= 0 && this.hoveredPosition) {
      ...
      if (
        brick &&
        this.gameManager.canPlayerPlaceBrick(
          this.selectedBrickIndex,
          this.hoveredPosition.row,
          this.hoveredPosition.col,
        )
      ) {
        this.showBrickPreview(
          brick,
          this.hoveredPosition.row,
          this.hoveredPosition.col,
        );
  ...
  private createBrickDisplay(brick: Brick, index: number): Container {
    ...
    bg.stroke({
      width: 2,
      color: this.selectedBrickIndex === index ? 0xffff00 : 0x666666,
    });
