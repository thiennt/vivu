...
export class StatsSelection extends Container {
    ...
    public onSelection: (character: Player | Enemy, statsName: string, statsValue: number) => void = () => {};
    ...
    public addSelection(statsName: string, statsValue: number, character: Player | Enemy) {
        let pos = character.position;
        let posX = pos.x;
        let posY = pos.y - 110;
        
        // Card Stats
        let icon = new Sprite(Assets.get(statsName));
        icon.anchor = 0.5;
        //icon.scale = 0.7;
        icon.width = 36;
        icon.height = 36;
    ...
    public addPlayerStatsSelection(character: Player | Enemy) {
        let statsName = getRandomItems(["atk", "def", "hp", "crit", "agi"], 1)[0];
        let statsValue = this.getstatsValue(statsName)?.value;
