...
export class DuelScene extends Container {
  ...
  constructor() {
    ...
    this.battleHistory = [
      {
        turn: 1,
        character_id: this.player,
        targeted_character_id: this.enemy,
        action_name: "fight",
        is_missed: false,
        damage: 1,
      },
      {
        turn: 2,
        character_id: this.enemy,
        targeted_character_id: this.player,
        action_name: "crit",
        is_missed: false,
        damage: 7,
      },
  ...
  public async startBattle() {
    ...
    console.log(
      `Turn ${action.turn}: ${action.character_id === this.player ? "Player" : "Enemy"} performs ${action.action_name} on ${action.targeted_character_id === this.player ? "Player" : "Enemy"} with damage ${action.damage}`,
    );
