...
export class CombatScene extends Container {
  ...
  private useDataDrivenSkills: boolean = true; // Enable the new skill system
  ...
  public async show() {
    for (let i = 0; i < this.battleHistory.length; i++) {
      ...
      let skillId = action.skill_id;
      let direction = action.direction;

      if (!attacker.isDie()) {
        await waitFor(1);

        // Use data-driven skill system if enabled and skill_id is provided
        if (this.useDataDrivenSkills && skillId) {
          await attacker.executeSkill(skillId, targets, direction);
        } else {
