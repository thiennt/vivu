import { AnimatedSprite, Assets, Container, Graphics, Sprite, Text, Ticker } from 'pixi.js';
...
export class CombatScene extends Container {
    ...
    constructor() {
        ...
        this.monster1 = new Monster({ rarity: 'apprentice', name: 'pipo-enemy001.png' });
        this.addChild(this.monster1);

        this.monster2 = new Monster({ rarity: 'expert', name: 'pipo-enemy002.png' });
        this.addChild(this.monster2);

        this.monster3 = new Monster({ rarity: 'master', name: 'pipo-enemy003.png' });
        this.addChild(this.monster3);

        this.monster4 = new Monster({ rarity: 'grandmaster', name: 'pipo-enemy004.png' });
        this.addChild(this.monster4);
        
        this.battleHistory = [
            { round: 1, turn: 1, attacker: this.player1, targets: [ { target: this.monster1, is_hit: false, damage: 3 } ], action_name: 'unleashSlash', direction: 'up' },
            { round: 1, turn: 2, attacker: this.monster1, targets: [ { target: this.player1, is_hit: false, damage: 1 } ], action_name: 'unleashFire', direction: 'down' },
            { round: 1, turn: 3, attacker: this.player2, targets: [ { target: this.monster2, is_hit: false, damage: 4 }, { target: this.monster3, is_hit: false, damage: 7 } ], action_name: 'unleashWind', direction: 'up' },
            { round: 1, turn: 4, attacker: this.monster2, targets: [ { target: this.player2, is_hit: false, damage: 1 } ], action_name: 'unleashThunder', direction: 'down' },
            { round: 2, turn: 5, attacker: this.player1, targets: [ { target: this.monster1, is_hit: false, damage: 6 }, { target: this.monster3, is_hit: false, damage: 6 } ], action_name: 'unleashSlash', direction: 'up' },
            { round: 2, turn: 6, attacker: this.monster1, targets: [ { target: this.player1, is_hit: false, damage: 4 } ], action_name: 'unleashFire', direction: 'down' },
            { round: 2, turn: 7, attacker: this.player2, targets: [ { target: this.monster2, is_hit: false, damage: 4 }, { target: this.monster3, is_hit: false, damage: 7 } ], action_name: 'unleashWind', direction: 'up' },
            { round: 2, turn: 8, attacker: this.monster2, targets: [ { target: this.player2, is_hit: false, damage: 3 } ], action_name: 'unleashThunder', direction: 'down' },
        ];
    }
    ...
    public checkDamages(targetRows: { target: Hero | Monster, is_hit: boolean, damage: number }[]) {
    ...
    public dieFlash(target: Hero | Monster) {
        ...
        const animation = new AnimatedSprite(
            [textures.explosion_1, textures.explosion_2, textures.explosion_3, textures.explosion_4, textures.explosion_5]
        );
        animation.x = targetX;
        animation.y = targetY;
