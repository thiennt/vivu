...
export class AIPlayer {
    ...
    public selectAndPlaceBrick(): { brickIndex: number; row: number; col: number } | null {
        ...
        let bestMove: { brickIndex: number; row: number; col: number; score: number } | null = null;

        for (let brickIndex = 0; brickIndex < availableBricks.length; brickIndex++) {
        ...
        if (bestMove) {
            return { brickIndex: bestMove.brickIndex, row: bestMove.row, col: bestMove.col };
        }
    ...
    private evaluatePlacement(brick: Brick, row: number, col: number): number {
        ...
        const tempBoard = gameState.board.map(row => row.map(cell => ({ ...cell })));
    ...
    private scoreLineCompletions(board: any[][]): number {
        ...
        for (let row = 0; row < 8; row++) {
            const aiCells = board[row].filter(cell => cell.owner === Player.AI).length;
            const humanCells = board[row].filter(cell => cell.owner === Player.HUMAN).length;
        ...
        for (let col = 0; col < 8; col++) {
            const aiCells = board.map(row => row[col]).filter(cell => cell.owner === Player.AI).length;
            const humanCells = board.map(row => row[col]).filter(cell => cell.owner === Player.HUMAN).length;
            const emptyCells = 8 - aiCells - humanCells;
    ...
    private scoreProximityToAIPieces(board: any[][], row: number, col: number, brick: Brick): number {
        let score = 0;
